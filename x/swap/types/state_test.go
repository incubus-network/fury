...
func TestState_NewPoolRecord(t *testing.T) {
	reserves := sdk.NewCoins(musd(50e6), ufury(10e6))
	...
	assert.PanicsWithValue(t, "reserves must have two denominations", func() {
		reserves := sdk.NewCoins(ufury(10e6), hard(1e6), musd(20e6))
		_ = types.NewPoolRecord(reserves, totalShares)
...
func TestState_NewPoolRecordFromPool(t *testing.T) {
	...
	assert.Equal(t, types.PoolID("ufury", "musd"), record.PoolID)
	assert.Equal(t, ufury(10e6), record.ReservesA)
	assert.Equal(t, record.ReservesB, musd(50e6))
	assert.Equal(t, pool.TotalShares(), record.TotalShares)
	assert.Equal(t, sdk.NewCoins(ufury(10e6), musd(50e6)), record.Reserves())
...
total_shares: "3000000"
`
	record := types.NewPoolRecord(sdk.NewCoins(ufury(1e6), musd(5e6)), i(3e6))
...
func TestState_PoolRecord_Validations(t *testing.T) {
	validRecord := types.NewPoolRecord(
		sdk.NewCoins(musd(500e6), ufury(100e6)),
		i(300e6),
	)
	testCases := []struct {
		name        string
		...
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ufurymusd' is invalid",
		},
		{
		...
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID ':musd' is invalid",
		},
		{
		...
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'musd:ufury' is invalid",
		},
		{
		...
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ufury:musd' does not match reserves",
		},
		{
		...
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ufury:musd' has invalid reserves: -1ufury",
		},
		{
			name:        "zero reserve a",
			poolID:      "ufury:musd",
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ufury:musd' has invalid reserves: 0ufury",
		},
		{
			name:        "negative reserve b",
			poolID:      "ufury:musd",
			reservesA:   validRecord.ReservesA,
			reservesB:   sdk.Coin{Denom: "musd", Amount: sdkmath.NewInt(-1)},
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ufury:musd' has invalid reserves: -1musd",
		},
		{
			name:        "zero reserve b",
			poolID:      "ufury:musd",
			reservesA:   validRecord.ReservesA,
			reservesB:   sdk.Coin{Denom: "musd", Amount: sdk.ZeroInt()},
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ufury:musd' has invalid reserves: 0musd",
		},
		{
			...
			expectedErr: "pool 'ufury:musd' has invalid total shares: -1",
		},
		{
			...
			expectedErr: "pool 'ufury:musd' has invalid total shares: 0",
		},
	}
...
func TestState_PoolRecord_OrderedReserves(t *testing.T) {
	invalidOrder := types.NewPoolRecord(
		...
		sdk.Coins{musd(500e6), ufury(100e6)},
		i(300e6),
	)
	...
	validOrder := types.NewPoolRecord(
		// force order to not be sorted
		sdk.Coins{ufury(500e6), musd(100e6)},
		i(300e6),
	)
	assert.NoError(t, validOrder.Validate())

	record_1 := types.NewPoolRecord(sdk.NewCoins(musd(500e6), ufury(100e6)), i(300e6))
	record_2 := types.NewPoolRecord(sdk.NewCoins(ufury(100e6), musd(500e6)), i(300e6))
	// ensure no regresssions in NewCoins ordering
	assert.Equal(t, record_1, record_2)
	assert.Equal(t, types.PoolID("ufury", "musd"), record_1.PoolID)
	assert.Equal(t, types.PoolID("ufury", "musd"), record_2.PoolID)
}
...
func TestState_PoolRecords_Validation(t *testing.T) {
	...
	assert.Error(t, err)
	assert.EqualError(t, err, "pool 'ufury:musd' has invalid total shares: -1")
}
...
func TestState_PoolRecords_ValidateUniquePools(t *testing.T) {
	...
	record_2 := types.NewPoolRecord(
		sdk.NewCoins(musd(5000e6), ufury(1000e6)),
		i(3000e6),
	)

	record_3 := types.NewPoolRecord(
		sdk.NewCoins(musd(5000e6), hard(1000e6)),
		i(3000e6),
	)
	...
	assert.EqualError(t, invalidRecords.Validate(), "duplicate poolID 'ufury:musd'")
}

func TestState_NewShareRecord(t *testing.T) {
	depositor := sdk.AccAddress("some user")
	poolID := types.PoolID("ufury", "musd")
...
`
	...
	record := types.NewShareRecord(depositor, "ufury:musd", i(3e6))
	data, err := yaml.Marshal(record)
	require.NoError(t, err)
...
func TestState_ShareRecord_Validations(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedErr: "depositor 'fury1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and pool 'ufury:musd' has invalid total shares: -1",
		},
		{
			name:        "zero total shares",
			depositor:   validRecord.Depositor,
			...
			expectedErr: "depositor 'fury1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and pool 'ufury:musd' has invalid total shares: 0",
		},
	}
...
func TestState_ShareRecords_Validation(t *testing.T) {
	...
	err = records.Validate()
	assert.Error(t, err)
	assert.EqualError(t, err, "depositor 'fury1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and pool 'hard:musd' has invalid total shares: -1")
}
...
func TestState_ShareRecords_ValidateUniqueShareRecords(t *testing.T) {
	...
	record_1 := types.NewShareRecord(depositor_1, "ufury:musd", i(20e6))
	record_2 := types.NewShareRecord(depositor_1, "ufury:musd", i(10e6))
	record_3 := types.NewShareRecord(depositor_1, "hard:musd", i(20e6))
	record_4 := types.NewShareRecord(depositor_2, "ufury:musd", i(20e6))
	...
	assert.EqualError(t, invalidRecords.Validate(), "duplicate depositor 'fury1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and poolID 'ufury:musd'")
}
