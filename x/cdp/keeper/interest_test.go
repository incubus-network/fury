	...
	testCases := []test{
		{
			...
			args{
				...
				initialPrincipal:        c("musd", 100000000000),
				timeElapsed:             oneYearInSeconds,
				expectedFees:            c("musd", 5000000000),
		...
		},
		{
			...
			args{
				...
				initialPrincipal:        c("musd", 10000000),
				timeElapsed:             7,
				expectedFees:            c("musd", 0),
...
func (suite *InterestTestSuite) TestMultipleCDPInterest() {
	...
	testCases := []test{
		{
			...
			args{
				...
				initialCDPPrincipal:          c("musd", 500000000),
				numberOfCdps:                 100,
				expectedFeesPerCDP:           c("musd", 5),
				expectedTotalPrincipalPerCDP: c("musd", 500000005),
				expectedFeesUpdatedTime:      time.Date(2020, 12, 15, 14, 0, 0, 0, time.UTC).Add(time.Duration(int(time.Second) * 7)),
		...
		},
		{
			...
			args{
				...
				initialCDPPrincipal:          c("musd", 500000000),
				numberOfCdps:                 100,
				expectedFeesPerCDP:           c("musd", 541),
				expectedTotalPrincipalPerCDP: c("musd", 500000541),
		...
		},
		{
			...
			args{
				...
				initialCDPPrincipal:          c("musd", 500000000),
				numberOfCdps:                 100,
				expectedFeesPerCDP:           c("musd", 54152),
				expectedTotalPrincipalPerCDP: c("musd", 500054152),
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			musdSupply := bk.GetSupply(suite.ctx, types.DefaultStableDenom)
			...
			suite.Require().Equal(tc.args.expectedStableBalance, musdSupply.Amount)
	...
	testCases := []test{
		{
			...
			args{
				...
				minPrincipal:       c("musd", 100000000),
				principalIncrement: c("musd", 10000000),
				initialTime:        time.Date(2020, 12, 15, 14, 0, 0, 0, time.UTC),
				timeElapsed:        oneYearInSeconds,
				expectedCDPs:       10,
			},
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			for j, addr := range addrs {
				initialPrincipal := tc.args.minPrincipal.Add(c("musd", int64(j)*tc.args.principalIncrement.Amount.Int64()))
