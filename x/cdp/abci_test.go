...
func (suite *ModuleTestSuite) createCdps() {
	...
	for j := 0; j < 100; j++ {
		...
		suite.Nil(suite.keeper.AddCdp(suite.ctx, addrs[j], c(collateral, int64(amount)), c("musd", int64(debt)), collateral+"-a"))
...
func (suite *ModuleTestSuite) TestSeizeSingleCdpWithFees() {
	err := suite.keeper.AddCdp(suite.ctx, suite.addrs[0], c("xrp", 10000000000), c("musd", 1000000000), "xrp-a")
	suite.NoError(err)
	suite.Equal(i(1000000000), suite.keeper.GetTotalPrincipal(suite.ctx, "xrp-a", "musd"))
	ak := suite.app.GetAccountKeeper()
	bk := suite.app.GetBankKeeper()
